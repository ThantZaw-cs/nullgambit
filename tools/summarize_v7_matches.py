"""Audit the fixed candidate/v4 control, including original PGNs and per-move traces."""
...
def summarize(directory: Path) -> dict[str, Any]:
    ...
    assert report["opponent_sha256"] == CANDIDATE_SHA256
    for key in ("candidate_sha256", "opponent_sha256"):
        assert report[key] == plan[key]
    assert (plan["base_ms"], plan["increment_ms"]) in (
        (10_000, 100), (10_000, 500), (120_000, 500)
    )
    ...
    assert (report["status"] == "completed") == (len(rows) == requested)
    ...
    for index, row in enumerate(rows):
        ...
        assert game.headers["White"] == (plan["candidate_name"] if row["candidate_white"] else "v4")
        assert game.headers["Black"] == ("v4" if row["candidate_white"] else plan["candidate_name"])
        assert (
            game.headers["Result"]
        ...
        for entry, charged in zip(moves, clock_rows, strict=True):
            side = entry["side"]
        ...
        if row["termination"] not in FAILED_TERMINATIONS | {"ply_cap_draw"}:
    ...
    return {
        ...
        "interpretation": "Source-reserved opening pairs, one opponent. "
