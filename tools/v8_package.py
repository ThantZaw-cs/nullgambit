...
print(json.dumps({'sha256':hashlib.sha256(open('agent.py','rb').read()).hexdigest(),
 ...
 'counter_size':agent.COUNTER_SIZE,'search_stats':agent.LAST_SEARCH_STATS,
 'options':{'tt':agent.USE_SCORE_TT,'lmr':agent.USE_LMR}}))
"""
...
def main() -> None:
    ...
    report: dict[str, object] = {
        "scope": (
            "Current source compatibility artifact; not strength selection or release approval"
        ),
    ...
    with tempfile.TemporaryDirectory(prefix="v8-package-") as temporary:
        ...
        for index in range(2):
            dirs = (extracted, ROOT / "baselines/online_v7")
            if index:
                dirs = dirs[::-1]
