...
## v4 上线后诊断进行中（2026-09-08 09:15 UTC 开始）

- 从干净 `astra-v4` / `b32c1c574d21f97d833e102c2176670f1a6aa82a` 开始，最多约两小时。
  v4 另冻结于 `baselines/online_v4`，SHA-256 仍为
  `7f451006dd0d00ec29a0f10bca02f4f9301cf27e72547509bab6858816db87c4`；
  v3 仍为 `b08ab63e695f971e6322353eb5bba4657c91e901ac9b0b6fb9a326f1def56297`。
- 新增 61、62 轮 PGN/LOG 及 v4 validation log 已收到，原件只在忽略的本地目录中。
  第 60 轮明确早于 v4；61、62 无直接版本标识，不能由结束时间或初始化耗时归属版本。
  数据导入工具去除了初始化耗时推断版本的提示；引擎、时间策略和裁判均不改。
- Linux 正式对照预案在 `benchmarks/astra-v4-diagnosis/plan.json`：预定 10 个已用回归开局，
  换色 20 局，120+0.5，同一 Linux 主机、单 CPU affinity、同依赖，串行执行。
  最多运行 95 分钟；不按胜负提前停止、不按结果重试。保留原始裁判结果及所有逐步日志。
  若资源/预算使批次不完整，明确报告实际局数，不以 CI 冒烟成绩替代。
- Mac 阶段已完成：从实际 FEN 与完整双方历史重放 60/61/62 轮，共 284 次真实入口调用，
  均合法、未超传入棋钟、历史核对一致；所有重复和搜索中止结果均保留在本地。
  61/24 的 Rb6 与 61/35 的 Nc8 是明确复查窗口；62/29 的 exd5 更依赖搜索深度，
  62/38 的 Bd3 差异具有时间敏感性。两版完成的同深度分支分数一致，尚未建立 v4 特有缺陷。
  56 项测试、ruff、strict mypy 通过。详情见 `benchmarks/astra-v4-diagnosis/REVIEW.md`。
- [Linux 正式批次](https://github.com/ThantZaw-cs/nullgambit/actions/runs/34209500665)
  正在运行固定的 20 局预案，正式战绩尚未取得。最终保留/回退建议等待实际结果。
- 本轮不新增算法、不上传比赛、不合并 PR、不付费；不能预判 v4 更强。
