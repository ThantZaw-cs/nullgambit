...
**结论：可暂时继续 v4，目前没有充分证据建议回退 v3；也不能宣称已证明更强。**

- 从干净 `astra-v4` / `b32c1c574d21f97d833e102c2176670f1a6aa82a` 开始，本轮不改引擎算法、
  评价、时间策略或裁判，不上传比赛、不合并 PR、不付费。候选另冻结于 `baselines/online_v4`。
  v4 SHA-256：`7f451006dd0d00ec29a0f10bca02f4f9301cf27e72547509bab6858816db87c4`；
  v3 SHA-256：`b08ab63e695f971e6322353eb5bba4657c91e901ac9b0b6fb9a326f1def56297`。
  原型、v3、根目录旧版及 `harness/` 保持原样，16 份既有原始资料逐一核对哈希不变。
- 第 60 轮明确早于 v4；61、62 轮缺直接版本标识，均保留 `unknown`。时间已统一为 UTC；
  v4 验证日志日历日期由资料日期和上下文推定，其正文仅有时分秒。不能以结束时间、初始化
  或 Mac 着法相似性认定版本。原始 PGN/LOG/ZIP 与完整线上复盘仅在忽略的本地目录保存。
- 从真实 FEN、双方完整历史重放 60/61/62（102/104/87 plies）；无非法着或超时，均正常将死。
  最早有实测支持的复查窗：61/24 的 Rb6、62/29 的 exd5；61/35 的 Nc8、62/38 的 Bd3 更明显。
  本引擎深搜仅作诊断。两版完成的同深度分支分数一致，暂未建立 v4 特有回退或程序缺陷。
- Mac 共 **284 次线上局面真实入口调用**，均合法、历史一致、低于传入棋钟。62/38 九次纯入口
  复测，v3 两次 Bd3/d8、七次 Kf4/d9；v4 九次 Kf4/d9，不能把一次深度边界波动当作版本差异。
  22 次追加同深度分支诊断保留全部结果：21 完成、1 达到 45 秒上限，未填值或挑选重跑。
- [Linux 正式对照](https://github.com/ThantZaw-cs/nullgambit/actions/runs/34209500665)：固定
  **10 个已用回归开局，换色 20/20 局，120+0.5；v4 +9 =7 -4（62.5%），失败/VOID/封顶裁决均 0**。
  09:21:13–10:41:54 UTC，80 分 41.565 秒；未因比分提前停止或重跑。13 将死、6 三次重复、
  1 子力不足；2098 plies 及逐步时钟全部核验。配对 bootstrap 95% 为 45%–77.5%，p=0.125。
  六对正向、一对负向、三对持平；**Slav 换色双败保留为负向信号**，不是未见验证集。
- Linux 实机为 AMD EPYC 7763 / x86_64 / Python 3.12.3，四个可见 CPU，但整个进程树固定 CPU 0，
  数值库线程数均 1，同锁定依赖；未施加 2 GB 配额或官网完整隔离。此为 Linux 主机对照，
  不冒充官网完整容器下的配对验证。原始环境说明复用了冒烟模板，实际 600-ply 参数以预案
  和执行记录为准；原文件不改，报告已明确纠正该说明。
- Slav 失利另作 **24 次 Mac 局面探针**，不重开对局、不改变正式成绩：三个局面两版同着；
  另一局面已知 Linux v4 的 Nxe4 在 Mac v4 变为 Qxd1+，同深度两版均偏好后者，仍更像搜索
  地平线问题。有限抽查不能穷尽两场失利原因，详见 `benchmarks/astra-v4-diagnosis/REVIEW.md`。
- Mac 完整 56 项测试、ruff、strict mypy 通过；无私有资料/项目 venv 的独立导出目录也通过，
  复用已安装的 Mac 依赖。[同批次 Linux 兼容性 CI](https://github.com/ThantZaw-cs/nullgambit/actions/runs/34209504655)
  已通过 56 项测试、ruff、strict mypy 和候选 runner 冒烟；冒烟不计入上述 20 局。
- 结果保存在 `benchmarks/astra-v4-diagnosis/`；全部 46 个原始 Linux artifact 文件逐字节保留，
  ZIP 不进 Git。下一项仅建议验证“通路兵/升变防守的有界搜索延伸”假设，先留独立残局集；
  本轮没有实现该改动。待补证据：61/62 逐局构建关联、未参与调优的 Linux 验证开局。

以下保留此前阶段记录；当前结论以本次上线后诊断为准。
...
新候选的 Linux 比赛环境验证，不宣称普遍棋力或线上 Elo 已确认提升。本轮约 80 分钟，未上传。

- 在 `astra-v4` / `639fc47922b49d363962a5c0a73a7b4e69192f49` 接管，开始仅有未跟踪的
  `data/`；没有覆盖已有改动。用户提供的 v3 ZIP 与修改前原型逐字节一致，SHA-256 为
  `b08ab63e695f971e6322353eb5bba4657c91e901ac9b0b6fb9a326f1def56297`，已冻结到
  `baselines/online_v3/`。根目录旧版 `agent.py` 和 `harness/` 均未改。
- 已合法重放并匹配第 58、60、54 轮 PGN/LOG，保留真实起始 FEN 和双方历史。CSV 38 条去重后
  仍为 38；54–60 轮仅能暂归“疑似 Numba v3”组（+4 =1 -2），其余 31 条为旧谱系迹象。
  缺逐局提交 ID/哈希，因此精确版本标注 `unknown`，没有把全部历史记成 v3 战绩。
- 重点发现：58/17 的 g4 和 54/27 的 h3 存在深度依赖的选择变化；深度 6 下记录着与最佳着差
  102 / 164 cp，均是本引擎评价下的复查证据，非外部标准答案。60 轮较早是逐渐恶化，36...h5
  后有明确将军换后链，但此前已明显劣势。54 轮结尾三次重复属实，不能视为程序错误。
- 唯一候选改动是有界安静着历史排序（兼顾无 TT 着时首着排序），评价、静态搜索、和棋逻辑、
  时间策略不变。源码 SHA-256：
  `7f451006dd0d00ec29a0f10bca02f4f9301cf27e72547509bab6858816db87c4`。
- 修改前 46 项测试、ruff、strict mypy 通过；候选新增线上战术、排序与数据导入回归后，完整
  **53 项测试**、ruff、strict mypy 均通过。12 个开发局面固定深度 5 分数全部与冻结 v3 相同，
  节点/耗时中位为其
  37.9% / 36.1%，2.5 秒平均完成深度由 5.08 升为 5.83。
- 候选对冻结线上 v3 的 20 局快筛（10 秒 + 0.1 秒，逐开局交换黑白、独立进程串行）完成：
  **+7 =8 -5，55%，失败/VOID 0**，所有 PGN 已复放核验，配对 bootstrap 95% 区间 40%–67.5%。
  耗时 10 分 36 秒；两侧全用冻结源码、同开局换色、同等时钟。
- 正式验证六个开局在改引擎前预留且未参与调优，完整 12 局（120 秒 + 0.5 秒）完成：
  **+6 =6 -0，75%，失败/VOID 0**，耗时 42 分 36 秒。六次将死、六次三次重复，无封顶裁决。
  候选哈希与快筛完全一致；32 局原始 PGN 均已重放验证合法。六对得分为
  `1, 1, 0.5, 0.75, 0.5, 0.75`，配对 bootstrap 95% 区间为 58.3%–91.7%；四对正向、两对持平，
  描述性双侧配对符号检验 p=0.125，不能把小样本区间当作普遍提升的证明。
  原始结果在 `benchmarks/astra-v4/{fast-screen,formal-validation}.{json,pgn,log}`；汇总在
  对应 `*-summary.json`。这六个开局现已使用，后续调优不能再称其为未见验证集。
- 本机是 Mac arm64 / Python 3.12.13 / numpy 2.5.2 / numba 0.67.0 / chess 1.11.2。
  最新官方规则已获取，初始化等常量与旧 harness 有差别，详见保存的规则正文与
  `benchmarks/astra-v4/REVIEW.md`。没有新的 Linux 候选验证；现有 Linux `valid` 日志只对应 v3。
- 本地候选包 `artifacts/astra-v4-history-7f451006.zip` 仅含根目录 `agent.py` 与 `LICENSE`，
  源码逐字节核对通过；独立 runner 初始化 6.042 秒，三次入口检查均合法且低于传入时钟，
  无 stderr。Mac runner 峰值 RSS 约 276.4 MiB（未施加 Linux 内存配额）。原始记录为
  `benchmarks/astra-v4/package-runtime.json`。`tools/check_package.py --source` 支持明确指定候选，
  避免默认使用根目录旧版；本轮没有运行会打包旧根实现的 `make zip`。
- 线上原件及逐步诊断均仅保存在被忽略的 `data/online/`。暂缺新版赢棋 PGN/LOG 抽查样本和
  逐局版本关联记录。没有推送、合并、上传比赛、付费或引入第三方引擎/权重。
- 下一步：补赢棋原谱与版本关联，完成 Linux 固定环境的独立验证，再用新的未调优开局扩大
  候选对线上 v3 的配对样本。当前保留候选源码和本地 ZIP，不替换线上构建。
...
## PR #1 独立审查修复（2026-09-07）

- 审查版本 `3bf36a6` 在当前聚合后的本地历史中无法解析；以审查指出的函数和原始候选哈希定位并
  复现问题。`_root_search` 先把根局面写到 `history[history_len]`，三个进入子节点的 `_negamax`
  调用却仍传 `history_len`。现全部传 `history_len + 1`，避免第一个子节点覆盖根记录；没有改搜索
  算法、评价或根目录稳定版。
- 新回归直接调用真实 `_root_search`：先让当前局面在历史中出现一次，根写入成为第二次，再用 TT
  走法排序强制合法四步循环回到根，断言第三次重复计数发生。相同根搜索再跑一次用于覆盖 PVS/不同
  分支的 scratch 重用，过期 deadline 覆盖超时路径；三轮均断言棋盘、状态、历史前缀和根槽不变。
  该测试在旧长度传递下计数为 0 并失败，修复后通过。
- 保留 1--500 ms 低钟测试，并新增真实 `get_move` 的 700、800、1000、2000 ms × 3 次长历史测试。
  12 次均实际完成搜索深度（3/3/3/4）且合法，总入口最大耗时约 25.1/24.5/34.4/65.7 ms。独立
  runner 在复杂局面连续四次协议往返约 42.4/29.6/34.6/59.0 ms，全部小于剩余时间；runner 初始化
  28.90 秒。原始值保存在 `benchmarks/numba-kernel-root-history-timing.json`。
- Python 3.12.3 最小依赖验证使用独立 `/tmp/nullgambit-kernel-py312`，只请求锁文件中的固定版本
  `chess==1.11.2 numpy==2.5.2 numba==0.67.0`，不请求 torch，也未改锁文件。安装在访问
  `https://pypi.org/simple/numba/` 时三次重试后因 `tunnel error: unsuccessful` 失败。因此最小依赖
  兼容验证和完整比赛环境验证均未完成；所需人工协助是提供这三个版本及传递依赖的 cp312 wheels，
  或临时开放该 PyPI 源（完整环境另需锁文件中的 PyTorch CPU wheel）。
- Python 3.14.4 下完整 unittest **46/46** 通过，ruff 与 strict mypy 通过。修复后候选 SHA-256 为
  `b08ab63e695f971e6322353eb5bba4657c91e901ac9b0b6fb9a326f1def56297`；稳定版与 Zaw 冻结对手仍为
  `887d8ca093d429d305cac98272d28447611bdcf07ac25ab5174c41ca6b6018f5`。
- 原来的 10 个 holdout 开局现明确作为**回归集**。修复版按 120 秒 + 0.5 秒、逐局换色、独立进程
  完整跑完 20 局：**+14 =4 -2，得分率 80.0%，失败 0，VOID 0**；16 次将死、3 次三次重复、1 次
  子力不足，总墙钟 70m15.853s。JSON、完整 PGN 和日志另存为
  `benchmarks/numba-kernel-root-history-formal-holdout.*`，未覆盖旧 `+9 =10 -1` 记录。该单批结果是修复
  回归证据，不据此宣称修复必然提升棋力。
